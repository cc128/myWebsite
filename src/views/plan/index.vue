...
<template>
    <div>
        ...
        <el-dialog title="编辑" :visible.sync="dialogVisible" width="80%" append-to-body :fullscreen="fullscreen" :close-on-click-modal="false">
            ...
            <div class="plan" v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.8)">
                    ...
                    <el-tree :data="data" :props="defaultProps">
                        <div class="custom-tree-node" style="flex: 1" slot-scope="{ node, data }">
                            <el-tooltip class="item" effect="dark" :content="node.label" placement="left">
                                <el-button :disabled="drawing[ELid] ? false : true" type="text" style="user-select: none;" @click="addEquipment(data)">{{ node.label }}</el-button>
                            </el-tooltip>
                        </div>
                    </el-tree>
                </div>
                <div class="plan-container" @mousemove="mousemove">
                    ...
                    <div class="full-screen">
                        ...
                        <br>
                        <span v-if="fullscreen" @click="fullscreen = true">
                        ...
                        <span v-else @click="fullscreen = false">
                ...
                <div v-if="drawing[ELid]" class="list">
                    ...
                    <div>锁定值{{ drawing[ELid].lock }}zIndex{{ zIndex }}</div>
                </div>
            </div>
    ...
    methods: {
        ...
        mousemove(e) {
            ...
            if (this.isRightKey && e.button === 0) {
                ...
                // if (left <= 0) left = 0;
                // if (top <= 0) top = 0;
                if (this.drawing[this.ELid].type === "children") {
                ...
                if (this.drawing[this.ELid].type === "parent") {
                    for (let i = 0; i < img.length; i++) {
                        ...
                        if (this.drawing[id].type === "children") {
                            // if ((this.drawing[id].parentX <= e.target.width - this.drawing[id].width / 2) && (this.drawing[id].parentY <= e.target.height - this.drawing[id].height * 0.75) && (this.drawing[id].parentX >= -(this.drawing[id].width / 2)) && (this.drawing[id].parentY >= -(this.drawing[id].height * 0.75))) {
        ...
        zoom(elScale) {
            ...
            let w = img.offsetWidth * elScale + img.offsetWidth;
            let l = img.offsetLeft - img.offsetWidth / 2 * elScale;
            let t = img.offsetTop - img.offsetHeight / 2 * elScale;
            img.style.left = l + "px";
            img.style.top = t + "px";
            img.style.width = w + "px";
            this.drawing[this.parentID].width = img.offsetWidth; //宽
            this.drawing[this.parentID].height = img.offsetHeight; //宽
            this.drawing[this.parentID].left = img.offsetLeft; //元素x位置
            this.drawing[this.parentID].top = img.offsetTop; //元素x位置
            this.drawing[this.parentID].resultX = img.offsetLeft; //最后的位置
            this.drawing[this.parentID].resultY = img.offsetTop; //最后的位置
        },
        // 点击缩小
        // 滚轮事件
        mousewheel(e) {
            if (e.deltaY > 0) {
                ...
                if (Object.keys(this.drawing[this.ELid].lock).length) {
                    if (e.target.offsetWidth <= this.drawing[this.ELid].lock.w) {
                        // 锁定时禁止缩小
                        return
                    } else {
                        this.elScale = -0.04
        ...
        upLayer(e) {
            ...
            if (Object.keys(this.drawing).length) {
                let img = document.getElementsByTagName("img");
            ...
            setTimeout(() => {
                this.$set(this.drawing, id, {
                    ...
                    top: 0, //上-坐标
                    lock: {}, //锁定值
                    width: this.planContainer.offsetWidth, // 宽
                    height: this.planContainer.offsetHeight, // 高
                    resultX: 0, // 最后的位置
                    resultY: 0, // 最后的位置
                });
                this.ELid = id; //当前元素id
                this.conEl = img; //当前元素
                img.style.width = this.drawing[id].width + "px";
                img.style.left = this.drawing[id].left + "px";
                img.style.top = this.drawing[id].top + "px";
                img.style.opacity = 1;
            }, 100);
